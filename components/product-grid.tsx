...
import { Star, ShoppingCart, Heart, Eye, Truck, Shield, ChevronLeft, ChevronRight, Loader2 } from "lucide-react"
...
export function ProductGrid({
  ...
  useEffect(() => {
    const loadWishlistStatus = async () => {
      ...
      const favoriteIds = wishlistStatus.filter((item) => item.inWishlist).map((item) => item.id)
              ...
              >
                <div className="relative">
                  ...
                  <div className="aspect-square bg-gradient-to-br from-primary/5 to-orange-50 p-6 flex items-center justify-center relative overflow-hidden">
                    ...
                    <img
                      src={product.image || `/placeholder.svg?height=200&width=200&query=${product.name} honey jar`}
                      alt={product.name}
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                      ...
                      {!product.inStock && (
                        <Badge variant="outline" className="bg-background/90 shadow-lg">
                          Out of Stock
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <Button asChild variant="secondary" size="sm" className="bg-white hover:bg-white shadow-lg">
                  ...
                  <CardContent className="p-5">
                    ...
                    {/* Product Info */}
                    <Link href={`/shop/${product.id}`} className="block group-hover:text-primary transition-colors">
                      <h3 className="font-semibold text-foreground mb-2 line-clamp-2 text-lg">{product.name}</h3>
                    </Link>
                    <p className="text-sm text-muted-foreground mb-4 font-medium">{product.weight}</p>
                    ...
                    <div className="flex gap-2">
                      <Button asChild variant="outline" size="sm" className="flex-1 bg-transparent hover:bg-muted">
                        <Link href={`/shop/${product.id}`}>
                          <Eye className="h-4 w-4 mr-2" />
                          View Details
                        </Link>
                      ...
                      <Button
                        ...
                        disabled={!product.inStock || loadingStates.cart[product.id]}
