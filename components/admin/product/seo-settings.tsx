...
export function SeoSettings({ form }: SeoSettingsProps) {
  ...
  useEffect(() => {
    let score = 0;
    const checks = [
      { condition: seoTitle && seoTitle.length >= 30 && seoTitle.length <= 60, points: 20 },
      { condition: seoDescription && seoDescription.length >= 120 && seoDescription.length <= 160, points: 20 },
      { condition: seoSlug && seoSlug.length >= 3 && seoSlug.length <= 50, points: 15 },
      { condition: seoKeywords && seoKeywords.length >= 3 && seoKeywords.length <= 8, points: 15 },
      { condition: seoTitle && productName && seoTitle.toLowerCase().includes(productName.toLowerCase()), points: 15 },
      { condition: seoKeywords && category && seoKeywords.some((k: string) => k.toLowerCase().includes(category.toLowerCase())), points: 15 }
    ];
  ...
  const generateSeoTitle = () => {
    ...
    const templates = [
      ...
      `Premium ${productName} - ${category || 'Product'} | Free Shipping Available`
    ];

    const template = templates[Math.floor(Math.random() * templates.length)];
    const truncated = template.length > 60 ? template.substring(0, 57) + '...' : template;
  ...
  const generateSeoDescription = () => {
    ...
    const templates = [
      `${shortDescription} Shop ${productName} with fast delivery and excellent customer service. Best quality ${category?.toLowerCase() || 'products'} at competitive prices.`,
      `Discover ${productName} - ${shortDescription} Free shipping available. Trusted ${category?.toLowerCase() || 'product'} supplier with 5-star reviews.`,
      `${productName}: ${shortDescription} Order now for quick delivery. Premium ${category?.toLowerCase() || 'products'} with satisfaction guarantee.`,
      `Buy ${productName} online. ${shortDescription} Secure payment, fast shipping, and excellent customer support available.`
    ];

    const template = templates[Math.floor(Math.random() * templates.length)];
    const truncated = template.length > 160 ? template.substring(0, 157) + '...' : template;
  ...
  const generateKeywords = () => {
    ...
    const baseKeywords = productName.toLowerCase().split(' ').filter(word => word.length > 2);
    const categoryKeywords = category.toLowerCase().split(' ').filter(word => word.length > 2);
  ...
  const removeKeyword = (index: number) => {
    const updatedKeywords = seoKeywords.filter((_: string, i: number) => i !== index);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-medium text-gray-900 mb-2">SEO & Search Optimization</h3>
        <p className="text-sm text-gray-500">
          Optimize your product for search engines to help customers find it easily.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <CardDescription>How well optimized your product is for search engines</CardDescription>
              </div>
            </div>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-sm">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                {seoTitle && productName && seoTitle.toLowerCase().includes(productName.toLowerCase()) ? 
                  <CheckCircle className="h-4 w-4 text-green-600" /> : 
                  <XCircle className="h-4 w-4 text-red-600" />
                }
              ...
              <div className="flex items-center space-x-2">
                {seoKeywords && category && seoKeywords.some((k: string) => k.toLowerCase().includes(category.toLowerCase())) ? 
                  <CheckCircle className="h-4 w-4 text-green-600" /> : 
                  <XCircle className="h-4 w-4 text-red-600" />
                }
      ...
      <Card className="bg-gray-50 border-gray-200">
        <CardHeader>
          ...
          <CardDescription>How your product will appear in Google search results</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="bg-white p-4 rounded-lg border">
            <div className="space-y-1">
              <div className="flex items-center space-x-2 text-sm text-green-700">
                ...
                <span>yourstore.com › products › {seoSlug || 'product-slug'}</span>
              </div>
