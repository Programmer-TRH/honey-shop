...
import { getProductReviews, markReviewHelpful } from "@/actions/review-actions";
  ...
  const handleMarkHelpful = async (reviewId: number) => {
    ...
    if (result.success) {
      ...
      setReviews((prev) =>
        prev.map((review) =>
          review.id === reviewId
            ? { ...review, helpful: result.helpful || review.helpful + 1 }
            : review
        )
      );
    }
                      ...
                      >
                        <ThumbsUp
                          className={`h-4 w-4 mr-2 ${
                            helpfulClicks[review.id] ? "fill-current" : ""
                          }`}
                        />
