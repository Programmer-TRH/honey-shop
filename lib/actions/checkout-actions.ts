...
import { getCart, clearCart } from "@/lib/actions/cart-actions"
...
export async function calculateDeliveryFee(city: string, deliveryMethod = "standard"): Promise<number> {
...
export async function placeOrder(orderData: OrderData) {
  try {
    ...
    if (cart.items.length === 0) {
      return { success: false, error: "Cart is empty", errors: ["Cart is empty"] }
    }

    // Validate required fields
    const requiredFields = ["firstName", "lastName", "phone", "address", "city", "area"]
    const missingFields = requiredFields.filter((field) => !orderData[field as keyof OrderData]?.trim())
...
export async function processCheckout(checkoutData: CheckoutData) {
  try {
    ...
    // Calculate delivery fee
    const deliveryFee = await calculateDeliveryFee(checkoutData.city, checkoutData.deliveryMethod)
    const total = cart.total + deliveryFee
    ...
    const order: Order = {
      ...
      estimatedDelivery: new Date(
        Date.now() + (checkoutData.deliveryMethod === "express" ? 1 : 3) * 24 * 60 * 60 * 1000,
      ).toISOString(),
    }
